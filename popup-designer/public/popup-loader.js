// Popup Loader - Hämtar config från Supabase
(function() {
  'use strict';
  
  // Supabase-konfiguration
  const SUPABASE_URL = 'https://wpvejgybuwpbyfecwtqk.supabase.co';
  const SUPABASE_ANON_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6IndwdmVqZ3lidXdwYnlmZWN3dHFrIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NjQ2MzY5MjIsImV4cCI6MjA4MDIxMjkyMn0.eidi-zuf0H86cDZv_Sd_pxk5Ne-j9GtxIcyDQpONltU';
  
  // Hämta popup ID från script URL
  ...
  if (!popupId) {
    console.error('Bythjul Popup: Inget popup ID angivet');
    return;
  }
  
  console.log('🔍 Hämtar popup config från Supabase:', popupId);
  
  // Hämta config från Supabase
  fetch(`${SUPABASE_URL}/rest/v1/popups?id=eq.${popupId}&select=config`, {
    headers: {
    ...
    .then(data => {
      if (!data || data.length === 0) {
        throw new Error('Popup config hittades inte');
      }
      
      const config = data[0].config;
      console.log('✅ Config hämtad från Supabase:', config);
      initPopup(config);
    })
    .catch(error => {
      console.error('❌ Kunde inte hämta popup config:', error);
    });
  ...
  function initPopup(config) {
    ...
    if (hasSubscribed) {
      console.log('✅ Användare har redan prenumererat');
      return;
    }
