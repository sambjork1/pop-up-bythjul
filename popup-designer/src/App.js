...
import { Copy, Check, Eye, Code, Palette, Save } from 'lucide-react';
...
export default function PopupDesigner() {
  ...
  const [savedPopupId, setSavedPopupId] = useState(null);
  ...
  const saveToSupabase = async () => {
    if (!popupName.trim()) {
      alert('Vänligen ange ett namn för popupen');
      return;
    }
    ...
    try {
      ...
      setSavedPopupId(data.id);
      alert(`✅ Popup sparad! ID: ${data.id}`);
    } catch (error) {
      console.error('Error saving popup:', error);
      alert('❌ Kunde inte spara popup: ' + error.message);
    } finally {
      setSaving(false);
    }
  };

  const generateShortScript = () => {
    if (!savedPopupId) {
      return '// Spara popupen först för att få det korta scriptet';
    }
    return `<!-- Bythjul Popup -->
<script>
(function() {
  'use strict';
  var script = document.createElement('script');
  script.src = '${config.baseUrl}/popup-loader.js?id=${savedPopupId}';
  script.defer = true;
  ...
  const copyShortScript = () => {
    navigator.clipboard.writeText(generateShortScript());
    setCopiedShort(true);
              ...
              <div className="space-y-3">
                <div>
                  ...
                  <input
                    ...
                    placeholder="t.ex. Vinter Kampanj 2024"
                    className="w-full px-3 py-2 border border-gray-300 rounded mb-2"
                  />
                  <button
                    onClick={saveToSupabase}
                    disabled={saving}
                    className="w-full bg-purple-600 text-white py-3 px-4 rounded-lg font-medium hover:bg-purple-700 transition flex items-center justify-center disabled:opacity-50"
                  >
                    {saving ? (
                      <>Sparar...</>
                    ) : (
                ...
                {savedPopupId && (
                  <div className="border-t pt-3">
                    <p className="text-sm text-gray-600 mb-2">
                      <strong>📦 Kort installations-script:</strong>
                    </p>
                    <button
